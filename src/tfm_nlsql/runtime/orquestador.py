...
ETAPA_CALENTAMIENTO = "Calentamiento de caché (D-34)"
ETAPA_GENERACION = "Generación (LLM)"
ETAPA_AST = "Validación AST"
ETAPA_EXPLAIN = "EXPLAIN (binding)"
ETAPA_EJECUCION = "Ejecución DuckDB"
ETAPA_REINTENTO = "Reintento (D-20)"
...
class ResultadoConsulta:
    ...
    veredicto: str = "pendiente"
    etapas: list[dict] = field(default_factory=list)
    reintentos: list[dict] = field(default_factory=list)
...
@contextmanager
def reservar_modelo():
    """Reserva el modelo para una sola consulta a la vez.

    `llama-server` sirve un contexto de 4.096 fichas: dos consultas
    simultáneas se pisan el `n_ctx` y la caché de prefijo de D-34. Rinde
    `False` si ya hay una en curso, sin esperar, para que quien llama avise
    en lugar de encolar. El cerrojo es del proceso, no de la sesión: dos
    pestañas de la interfaz comparten el mismo servidor (R-08).
    """
    reservado = _EN_CURSO.acquire(blocking=False)
    try:
        yield reservado
    finally:
        if reservado:
...
def modelo_ocupado() -> bool:
    return _EN_CURSO.locked()


@contextmanager
def _medir(etapas: list[dict] | None, nombre: str, detalle: str = ""):
    """Cronometra una etapa del recorrido para el panel de trazas (D-22)."""
...
def validar_para_ejecutar(
    ...
    with _medir(etapas, ETAPA_AST, "política sobre el AST y lista blanca"):
        validar_politica(reescrito, columnas)
    with _medir(etapas, ETAPA_EXPLAIN, "binding en DuckDB sin ejecutar"):
        validar_binding(con, reescrito)
    return reescrito


def precalentar() -> bool:
    """D-34: paga el prefill si el modelo ya escucha. No bloquea si no hay 7B."""
    global _calentado
    if _calentado:
        return True
...
def consultar(
    ...
    *,
    ...
    if calentar_cache and not _calentado:
        with _medir(etapas, ETAPA_CALENTAMIENTO, "prefill del contrato"):
            calentar(construir_prompt("calentamiento"))
        _calentado = True
    ...
    etapas.append(
        {
            "etapa": ETAPA_GENERACION,
            "ms": round((t_gen - t_calor) * 1000, 1),
            "ok": bool(sql_crudo or abstencion),
            "detalle": _fichas(primera),
        }
    )
        ...
        try:
            with conexion_lectura(gold or GOLD_DB) as con:
                sql_final, veredicto, error, reintentos, gen_corr = _ciclo(
                    pregunta, sql_crudo, columnas, con, etapas, max_intentos
                )
                ...
                if sql_final is not None:
                    with _medir(etapas, ETAPA_EJECUCION, "conexión de solo lectura"):
...
def _fichas(gen: RespuestaLLM) -> str:
    if gen.predicted_n is None:
        return ""
    return f"{gen.predicted_n} fichas generadas, {gen.prompt_n} de prompt"
...
def _ciclo(
    ...
    for intento in range(max_intentos):
        try:
            return (
                validar_para_ejecutar(actual, columnas, con, etapas),
                "aceptado",
                None,
                reintentos,
                gen_corr,
            ...
            with _medir(etapas, ETAPA_REINTENTO, e.motivo.splitlines()[0]):
