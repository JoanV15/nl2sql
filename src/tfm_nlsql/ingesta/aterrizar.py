...
from tfm_nlsql.ingesta.esquemas import CSV_FUNNEL, CSV_OBLIGATORIOS
from tfm_nlsql.rutas import LANDING

DATASET = "olistbr/brazilian-ecommerce"
DATASET_FUNNEL = "olistbr/marketing-funnel-olist"
...
def aterrizar_funnel() -> Path:
    import kagglehub

    origen = Path(kagglehub.dataset_download(DATASET_FUNNEL))
    LANDING.mkdir(parents=True, exist_ok=True)
    for nombre in CSV_FUNNEL:
        src = origen / nombre
        if not src.is_file():
            hallados = list(origen.rglob(nombre))
            src = hallados[0] if hallados else src
        if not src.is_file():
            raise SystemExit(
                f"La descarga de {DATASET_FUNNEL} no trajo {nombre}. Origen: {origen}"
            )
        shutil.copy2(src, LANDING / nombre)
    return LANDING
