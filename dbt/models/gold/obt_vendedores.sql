...
SELECT
  ...
  cap.fecha_captacion,
  cap.canal_captacion,
  cap.segmento_negocio,
  a.fecha_primera_venta,
  CAST(
    date_diff('day', cap.fecha_captacion, a.fecha_primera_venta) AS INTEGER
  ) AS dias_hasta_primera_venta,
...
LEFT JOIN {{ ref('stg_captacion') }} AS cap ON s.seller_id = cap.seller_id
