...
WITH base AS (
  ...
  LEFT JOIN {{ ref('stg_eventos_logisticos') }} AS e ON o.order_id = e.order_id
),
...
SELECT
  ...
  importe_pagado,
  COALESCE(importe_articulos, 0) * (SELECT eur_por_brl FROM {{ ref('stg_tipo_cambio') }}) AS importe_articulos_eur,
  tipo_pago_principal,
  num_plazos,
