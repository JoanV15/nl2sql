...
> ```
>
> Pregunta: ¿Qué categorías de producto vendieron más unidades en 2016 que en 2015?
> ```
> WITH u2015 AS (
>   SELECT categoria_producto, COUNT(*) AS unidades
>   FROM obt_lineas_pedido
>   WHERE es_venta_valida AND YEAR(fecha_compra) = 2015
>   GROUP BY categoria_producto
> ),
> u2016 AS (
>   SELECT categoria_producto, COUNT(*) AS unidades
>   FROM obt_lineas_pedido
>   WHERE es_venta_valida AND YEAR(fecha_compra) = 2016
>   GROUP BY categoria_producto
> )
> SELECT u2016.categoria_producto
> FROM u2016
> JOIN u2015 USING (categoria_producto)
> WHERE u2016.unidades > u2015.unidades;
> ```
>
> Pregunta: ¿Cuál es el ticket medio de los últimos tres meses?
> ```
> SELECT SUM(importe_articulos) / COUNT(*) AS ticket_medio
> FROM obt_pedidos
> WHERE es_venta_valida
>   AND fecha_compra >= CAST('2018-10-17' AS DATE) - INTERVAL 3 MONTH
>   AND fecha_compra < CAST('2018-10-17' AS DATE) + INTERVAL 1 DAY;
> ```
>
...
Los seis ejemplos son el bloque más caro por unidad y el primero a recortar
...
El cuarto (D-63) enseña el grano fino: `WITH` de dos años naturales,
`GROUP BY categoria_producto` y unidades, no `macro_categoria`. La pregunta
no está en el catálogo; los años 2015–2016 no son el 2018–2017 de la 27, y
el SQL no copia el de referencia.

El quinto (D-63) fija las ventanas relativas contra
`CAST('2018-10-17' AS DATE)`, nunca `CURRENT_DATE`. Tampoco es un ítem de
evaluación: «ticket medio de los últimos tres meses» no es la 35 ni la 36.

El sexto demuestra que la abstención es una salida legítima y fija su formato
...
| 4 · Reglas de negocio | ~350 | 474 | **Completo** (D-63: recorte de una frase; D-48, D-50) |
| 5 · Ausencias declaradas | ~180 | 218 | **Completo** (D-51 reseñas individuales) |
| 6 · Abstención | ~130 | 160 | **Completo** |
| 7 · Ejemplos | ~630 | 533 | **Completo** (D-63, seis ejemplos) |
| **Total** | **~2.980** | **3.332** | Ventana 4.096. Ocupación ~3.618. **Cabe.** |

**Verificación.** Recuento del 2026-09-07 con el endpoint `/tokenize` de
`llama-server` (Qwen2.5-Coder-7B-Instruct Q4_K_M; el tokenizer coincide
con el 3B: el prefijo 1.1 sigue midiendo 3.067). Concatenando los bloques
`[PREFIJO]` en el orden del documento, el prefijo que consume el runtime
mide **3.332 fichas**. La suma de los siete bloques, tokenizados por
separado, es 3.331. Respecto a 1.1: el bloque 7 añade 280 fichas (dos
ejemplos N2/N3); el bloque 4 pierde 16 al recortar la frase de productos
sin categoría, que restaba el doceavo valor. El umbral de alerta de D-46
(~3.200 de prefijo) se supera; el techo de parada de D-63 (~3.400) no.
...
3.332 (prefijo) + ~30 (pregunta) + 256 (respuesta, D-61) ≈ 3.618 de 4.096.

Con el reintento de D-20 (SQL fallido + error al final) la cabeza sigue
por debajo de 4.096. El coste del exceso respecto al sintético de P-07 lo
absorbe D-34 en el arranque. Con la caché caliente el tamaño del prefijo
es irrelevante: solo se procesa el delta.

**Recorte.** D-63: no se recortan los ejemplos nuevos. Se elimina una
frase del bloque 4 que no viajaba información distinta de la lista de
macros. El orden de D-37 (primero los ejemplos) no aplica: no se ha
rebasado la ventana.

**Si hay que crecer.** Por encima de ~3.400 fichas de prefijo D-63 manda
parar y preguntar, no relajar D-50 ni subir la ventana en silencio.
