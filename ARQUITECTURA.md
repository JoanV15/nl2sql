...
LLM local cuantizado, Qwen2.5-Coder en Q4_K_M servido por `llama.cpp` (D-17,
D-29). El modelo del sistema es el de 7B y el de 3B quedó como brazo de
ablación (D-53, posterior a la versión 2.0 de este documento). Sin frameworks
que encapsulen el núcleo (D-05): usar LlamaIndex
...
presupuesto que D-61 elevó a 256 al comprobar que 120 truncaba el 11 % de las
consultas evaluadas. Con la caché caliente el tiempo de respuesta lo domina la
generación: 15,1 fichas por segundo en la medición original del 3B y unas 4,8
sobre el sistema real, el 7B en WSL2 (C-09: las latencias no se comparan entre
sistemas).
...
2. **De política, sobre el AST.** Solo `SELECT` y `WITH`; tablas y columnas
   restringidas a la lista blanca de Gold; y rechazo del cruce sin condición de
   enlace —la coma en el `FROM`, el `CROSS JOIN` y el `JOIN` pelado—, porque un
   producto cartesiano devuelve una cifra plausible y falsa que las otras tres
   capas no ven (D-60).
...
(D-20). **Un solo reintento**: cada uno añade una generación completa, de modo
que duplica el peor caso —53 segundos con el presupuesto de D-61 sobre el 7B—
mientras la consulta mediana se queda en unos 9. El SQL corregido vuelve a pasar
...
declarado, junto al estado del modelo servido y de la capa Gold en uso. El
recorrido de cada consulta —generación, validación del AST, `EXPLAIN` de
*binding* y ejecución— se pinta con su latencia por etapa y sus fichas, que es
la misma traza que D-22 persiste.
...
Medido en P-07 con el **3B** cargado: `llama-server` sostiene 2,5 GB de memoria
privada y deja 6,4 GB libres. El sistema (D-53) es el 7B Q4_K_M (~4,7 GB el
GGUF). No mezclar las dos cifras (C-09).

**Ejecución secuencial.** Los perfiles de Docker Compose se activan por
separado (D-62): `consulta` (Streamlit + Gold) y `catalogo` (OpenMetadata).
La plataforma de datos y el runtime de inferencia no conviven. Es la
mitigación de C-03, R-01 y R-08. El 7B no entra en ninguna imagen.
