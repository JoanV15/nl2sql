...
# El protocolo se fijó con 120 fichas de respuesta y sus filas históricas se
# leen así. D-61 subió el presupuesto del runtime a 256; aquí no se toca, o las
# pasadas nuevas dejarían de ser comparables con las de 2026-09-01 (C-09).
N_PREDICT_P07 = 120
...
def main() -> int:
    ...
    def medir(prompt: str, cache: bool):
        return completar(prompt, n_predict=N_PREDICT_P07, cache_prompt=cache)

    filas = [
        _fila("A — en frío, sin caché", medir(p1, False)),
        _fila("B — misma pregunta, caché poblada", medir(p1, True)),
        _fila("C — caché caliente, pregunta distinta", medir(p2, True)),
        _fila("D — caché caliente, tercera pregunta", medir(p3, True)),
    ]
