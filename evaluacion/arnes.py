...
from tfm_nlsql.runtime.cliente import N_PREDICT, metadatos_servidor  # noqa: E402
...
BLOQUES: tuple[tuple[str, str, list[tuple[int, str]]], ...] = (
    ("nivel1", "Nivel 1", NIVEL1),
    ("nivel2", "Nivel 2", NIVEL2),
    ("nivel3", "Nivel 3", NIVEL3),
    ("logistica", "Logística", LOGISTICA),
    ("embudo", "Embudo web", EMBUDO),
    ("fuentes", "Captación y FX", FUENTES),
    ("abstencion", "Abstención", ABSTENCION),
)
BLOQUES_POR_DEFECTO = "nivel2,nivel3,logistica,embudo,fuentes,abstencion"
...
def evaluar_una(
    n: int, pregunta: str, con_modelo: bool, max_intentos: int = 2
) -> dict:
    ...
    if cabeza.startswith("ABSTENCION"):
        if not con_modelo:
            ...
            out["tipo"] = "abstencion"
            return out
        r = consultar(pregunta, calentar_cache=True, max_intentos=max_intentos)
...
def _metadatos(
    resultados: list[dict], *, max_intentos: int
) -> dict:
...
def elegir_bloques(texto: str) -> list[tuple[str, str, list[tuple[int, str]]]]:
    por_clave = {clave: (titulo, preguntas) for clave, titulo, preguntas in BLOQUES}
    elegidos = []
    for crudo in texto.split(","):
        ...
        if clave not in por_clave:
            raise ValueError(f"bloque desconocido: {clave}")
        titulo, preguntas = por_clave[clave]
        elegidos.append((clave, titulo, preguntas))
    if not elegidos:
        raise ValueError("ningún bloque")
    return elegidos


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="Evaluación D-50 por bloque (n_predict vigente en metadatos)"
    )
    p.add_argument(
        "--solo-referencia",
        action="store_true",
    ...
    p.add_argument(
        "--bloques",
        default=BLOQUES_POR_DEFECTO,
        help=(
            "Lista de bloques (coma). Claves: nivel1,nivel2,nivel3,"
            "logistica,embudo,fuentes,abstencion. "
            f"Por defecto: {BLOQUES_POR_DEFECTO}"
        ),
    )
    p.add_argument(
        "--sin-reintento",
        action="store_true",
        help="Ablación D-21: un solo pase del validador (max_intentos=1)",
    )
    ...
    if args.etiqueta and not _ETIQUETA.match(args.etiqueta):
        ...
        return 2
    try:
        bloques = elegir_bloques(args.bloques)
    ...
    max_intentos = 1 if args.sin_reintento else 2
    resultados = []
    ...
    for _clave, _titulo, preguntas in bloques:
        for n, pregunta in preguntas:
            ...
            try:
                r = evaluar_una(
                    n, pregunta, con_modelo=con_modelo, max_intentos=max_intentos
                )
    ...
    if not args.solo_referencia:
        informe["metadatos"] = _metadatos(resultados, max_intentos=max_intentos)
