...
(`Arquitectura.md`) y PostgreSQL (un borrador paralelo de la propuesta).
...
**Fecha:** 2026-06-28 · **Estado:** Firme · **Divergencia con la propuesta aprobada**
...
**Contexto.** Un primer borrador de la propuesta planteaba usar
...
**Enmienda (2026-09-05, D-62).** El asset de Dagster que compilaba el contrato
desde la API de OpenMetadata **no se implementa**. El runtime lee
`CONTRATO_SEMANTICO.md` (D-33). OpenMetadata es el perfil `catalogo`: linaje y
glosario. No entra en el prompt.
...
**Enmienda (Hito 3).** El detector de Spark es a grano registro
(`PERMISSIVE` + `_corrupt_record`). El aislamiento es a grano fichero: un
CSV con alguna fila corrupta no entra en Bronze. Aislar solo la fila queda
como techo documentado en `bronze.py`.
...
**Fecha:** 2026-07-02 · **Estado:** Firme · **Divergencia con la propuesta aprobada**
...
**Decisión.** El SQL corregido vuelve a pasar obligatoriamente por el
validador. El orquestador de aplicación media siempre: el validador nunca
dialoga directamente con el modelo.

**Enmienda (D-35, 2026-08-18).** Un único reintento (dos pases del validador:
el original y, si falla, el corregido). El texto original hablaba de tres
intentos; D-35 lo recortó por latencia y el código es `range(2)`. No hay un
tercer pase.
...
**Enmienda (2026-09-04).** La UI llama `precalentar()` al primer pintado si
`llama-server` responde en `/health`. Si el modelo no está, no se bloquea el
landing; el frío lo paga la primera `consultar()`, como antes.
...
**Fecha:** 2026-07-08 · **Estado:** Firme · **Origen: indicación de los tutores**
...
**Corrección 2026-08-13.** D-52 midió la ablación. D-53 fija el 7B como
...
**Fecha:** 2026-07-10 · **Estado:** Firme · **Anillo exterior**
...
herramienta, y un hueco así lo resuelve quien implemente sin haber cerrado
la decisión, de forma irreversible en la práctica. `uv` produce un fichero de
...
### D-42 · El registro de decisiones es la única fuente de gobernanza

**Fecha:** 2026-07-14 · **Estado:** Firme

**Contexto.** Durante el diseño convivían notas de trabajo, un borrador
de la propuesta y este registro. El borrador contradecía
decisiones ya firmes: ordenaba Polars (eliminado en D-04), prohibía la nube
(en contra de D-07 y D-28), fijaba 16 GB de memoria (corregido a 6–7 en C-03),
exigía un prompt minimalista (lo contrario de D-33), describía el sistema como
RAG y, sobre todo, mandaba implementar la seguridad **buscando palabras clave
en la cadena SQL, incluida `JOIN`**: un control falso que D-19 sustituye por
análisis del AST, y que habría bloqueado las subconsultas del nivel 3.

**Decisión.** `DECISIONES.md` es la única fuente de verdad del diseño. Cualquier
nota auxiliar se descarta si contradice el registro. No se versionan ficheros
de instrucciones de editores ni de asistentes.

**Justificación.** Un documento auxiliar desactualizado es más dañino que su
ausencia: se obedece sin señalar el conflicto. La gobernanza de lo que dirige
el desarrollo merece el mismo rigor que la del código.
...
y así se reporta. Esta decisión se registra el 2026-08-12, **antes** de
...
### D-59 · Solo Silver admite abfs; Landing y Gold se quedan en local

**Fecha:** 2026-08-27 · **Estado:** Firme

**Contexto.** D-07 dejó Landing, Bronze y Silver como ruta configurable,
con conmutación a ADLS Gen2 (`abfss://`) y Gold siempre en DuckDB local.
El Hito 6 implementa esa conmutación. Landing, Bronze y cuarentena son
copia de CSV; Spark ya escribe Silver. DuckDB puede no cargar las
extensiones `azure`/`delta` para `delta_scan` sobre abfs. Las banderas
opcionales (eventos, clickstream, funnel, FX) se derivaban de directorios
en disco.

**Decisión.** Landing, Bronze y cuarentena permanecen siempre en disco
local. Solo `TFM_SILVER_PATH` admite una URI abfs. Gold sigue siendo un
fichero DuckDB local (D-07); `TFM_GOLD_PATH` no admite abfs. La cuenta y
el contenedor van en la URI; no se inventan. Si DuckDB no puede
`delta_scan` abfs, el proceso PARA: no se lee el Silver local en
silencio. Con URI abfs, las banderas opcionales valen 0.

**Justificación.** El plan B local de D-07 cubre el calendario; no hace
falta llevar la copia CSV a la nube para demostrar la ruta configurable.
Leer el Silver de disco cuando la URI apunta a abfs falsearía el
experimento. Inventar cuenta o contenedor rompería D-28 (credenciales
fuera del código). Poner las banderas a 0 evita mezclar tablas opcionales
de disco con un Silver remoto.

**Alternativa descartada.** Conmutar Landing/Bronze/cuarentena a abfs.
Caer al Silver local si DuckDB no abre abfs. Inventar una cuenta de
almacenamiento por defecto.

---

### D-60 · El validador rechaza el cruce sin condición de enlace

**Fecha:** 2026-09-03 · **Estado:** Firme

**Contexto.** En la demostración del 15 de septiembre, la pregunta sobre el
porcentaje de clientes recurrentes devolvió **0,003 %** frente al **3,12 %**
real de C-05, y lo devolvió con el veredicto «aceptado». La traza 149 explica
por qué: el modelo escribió `FROM clientes, obt_pedidos`, un producto
cartesiano de 96.096 clientes por 99.441 pedidos que infla el denominador del
porcentaje unas mil veces. Las cuatro capas de D-19 lo dejaron pasar porque el
SQL es sintácticamente correcto, toca solo tablas de la lista blanca y hace
*binding* sin error: `EXPLAIN` valida nombres, no semántica (C-06). El
`predicted_n` de esa generación fue exactamente 120, el techo de D-35, de modo
que la causa raíz es una respuesta truncada que se quedó sin escribir el `ON`.

**Decisión.** La capa de política de D-19 rechaza todo cruce que no declare
condición de enlace: la coma en el `FROM`, el `CROSS JOIN` y el `JOIN` sin
`ON` ni `USING`. El motivo se reinyecta al modelo por el bucle de D-20 y
sugiere la alternativa: subconsulta escalar o `JOIN` con `ON`.

**Justificación.** Un resultado numérico plausible y silenciosamente falso es
el peor fallo posible en un sistema que un no técnico va a leer como respuesta
de negocio; es más grave que un rechazo, porque el rechazo se ve. La
comprobación es decidible sobre el AST, no cuesta latencia y no bloquea ninguna
de las consultas de referencia del catálogo: ninguna usa coma ni `CROSS JOIN`,
y el porcentaje se expresa con una subconsulta escalar. El `JOIN` con condición
sigue permitido: lo desaconsejado entre las cuatro OBT es la desnormalización
(D-11), no la operación.

**Límite conocido.** Detecta la condición ausente, no una condición
tautológica del tipo `ON 1 = 1`. Nunca se ha observado esa forma en las trazas.

**Pendiente, no decidido aquí.** La causa raíz es el techo de fichas de D-35.
Subir `n_predict` es decisión de Joan: cuesta latencia en toda consulta y D-35
la fijó con la medición de P-07. El validador convierte el síntoma silencioso
en un rechazo visible, pero no elimina la truncación.

**Alternativa descartada.** Dejarlo al criterio del prompt, que no es una
frontera de seguridad (D-19). Comparar el número devuelto con un rango
esperado, que exigiría conocer la respuesta de antemano.

---

### D-61 · El presupuesto de fichas de salida sube de 120 a 256

**Fecha:** 2026-09-04 · **Estado:** Firme · **Sustituye la cifra de D-35**

**Contexto.** D-35 fijó el presupuesto en 120 fichas con la medición de P-07:
15,1 fichas por segundo, unos 66 ms por ficha, de modo que 120 fichas eran casi
8 segundos. Esa medida es del 3B sobre Windows nativo y C-09 advierte que no se
compara entre sistemas. Sobre el sistema real —7B en WSL2 con seis hilos— los
informes del arnés dan **208 ms por ficha** de mediana, así que el mismo techo
cuesta 25 segundos, no 8. Y el techo se alcanza: de las 53 consultas evaluadas
con marca de tiempo, **6 terminaron exactamente en 120 fichas** (27, 35, 36, 38,
40 y 41), un 11 % del total. D-60 documenta a qué conduce eso: la truncación de
la traza 149 dejó un `JOIN` sin `ON` y produjo un porcentaje mil veces menor
que el real, con veredicto «aceptado».

**Medición del requisito.** Sobre las generaciones no truncadas del 7B, la
relación es de 3,22 caracteres por ficha. Aplicada al SQL de referencia del
catálogo, la consulta más exigente —la 27, categorías que más crecieron en 2018
frente a 2017— necesita unas **216 fichas**; el percentil 90 está en 122 y la
mediana en 59.

**Decisión.** `N_PREDICT` pasa de 120 a **256** fichas. El resto de D-35 no
cambia: el modelo sigue emitiendo solo SQL, con las mismas secuencias de parada,
y el bucle de autocorrección sigue acotado a un único reintento.

**Justificación.** 256 cubre con margen la consulta más larga del catálogo y las
seis que se truncaban. El techo es un límite, no un coste: la mediana de fichas
generadas es 42, unos 9 segundos, y no se mueve al subir el límite. Lo que
empeora es la cola: el peor caso de una generación pasa de 25 a 53 segundos, y
con el reintento de D-20, de 50 a 107. Se acepta ese peor caso porque afecta
solo a las consultas largas del Nivel 3 y porque la alternativa —seguir
truncando— produce SQL silenciosamente falso, que es el fallo que D-60 acaba de
cerrar. La ventana no se compromete: 3.067 fichas de prefijo más 256 de
respuesta caben en las 4.096 de `n_ctx`, incluso sumando el SQL fallido y el
error que el reintento añade al final del prompt.

**Consecuencia sobre las cifras publicadas.** Las precisiones por nivel de los
Hitos 1 y 2 se midieron con 120 fichas. Cualquier pasada nueva del arnés debe
declarar el presupuesto usado; comparar una pasada de 256 con una de 120 mezcla
dos configuraciones, como advierte C-11 sobre los saldos netos.

**Alternativa descartada.** Subir a 200, que dejaría la 27 aún truncada. Hacer
el presupuesto variable por pregunta, que exigiría clasificar la dificultad
antes de generar y añadiría una llamada al modelo. Quitar el techo, que deja la
latencia sin cota superior.

---

### D-62 · Compose por perfiles; OpenMetadata es catálogo, no runtime

**Fecha:** 2026-09-05 · **Estado:** Firme

**Contexto.** El Hito 6 aplazó OpenMetadata y Docker Compose porque el WSL de
desarrollo no tenía el motor Docker integrado (C-03, R-01). A 30 horas de la
entrega Joan exige ambos: linaje visible de la arquitectura y un despliegue que
el tribunal pueda levantar. Meter 7B, Spark, Kafka, Mongo y OpenMetadata en un
solo `compose up` viola C-03 (~6 GB de pico) y R-08.

**Decisión.** Docker Compose entra con **perfiles mutuamente excluyentes**:

1. **`consulta`.** Imagen de Streamlit + Gold montado (`TFM_GOLD_PATH`). El
   7B permanece en el anfitrión (`llama-server` + GGUF): 4,7 GB no viajan en
   git ni caben junto al catálogo.
2. **`catalogo`.** OpenMetadata (MySQL, Elasticsearch, servidor). Fuera del
   camino de la consulta (D-06). El linaje se ingiere desde el manifiesto dbt
   de Silver → Gold; Kafka, Mongo y Spark Bronze/Silver se declaran como
   servicios del catálogo, no como contenedores que el tribunal tenga que
   arrancar para preguntar.
3. **Plataforma.** Sigue el comando ya cerrado del Hito 5
   (`tfm-nlsql-plataforma`). No se reescribe Spark a Compose en esta ventana.

El contrato semántico sigue siendo `CONTRATO_SEMANTICO.md`. OpenMetadata **no**
compila el prefijo ni se consulta en inferencia (D-33, C-04). La frase de D-06
sobre un asset Dagster que compilara el contrato desde la API no se implementa:
el prefijo ya es el artefacto versionado; OM es la vista de linaje.

**Justificación.** El tribunal prueba el producto NL-to-SQL (perfil `consulta`)
o inspecciona el gobierno (perfil `catalogo`). Nunca ambos a la vez. El linaje
que el sistema ya produce —dbt sobre las cuatro OBT— es el que OM exhibe. El
GGUF y `datos/` siguen fuera de git.

**Alternativas descartadas.** Un único stack con modelo, Spark y OM: OOM
garantizado en 7,8 GB de WSL y en un portátil del tribunal. Compilar el
`[PREFIJO]` desde OM: invalida la caché de prefijo y las cifras de P-07.
Meter el 7B en la imagen: 4,7 GB y licencia de redistribución del peso.

---

### D-63 · Few-shot de patrones N2/N3 en el bloque 7; D-50 intacta

**Fecha:** 2026-09-07 · **Estado:** Firme

**Contexto.** Con el 7B, `n_predict` 256 y un reintento, el Hito 2 queda en
Nivel 2 **5/16** y Nivel 3 **1/11** (solo la 45), contrato 1.1, 3.067 fichas
(`evaluacion/resultados/hito2_7b_np256.json`). El bloque 7 ya tiene cuatro
ejemplos (~253 fichas): facturación 2018, ranking de vendedores, censo
`shipped` (D-49) y abstención de margen. Ninguno enseña un `WITH` de dos
años, la fecha de referencia frente a `CURRENT_DATE`, ni
`categoria_producto` frente a `macro_categoria`. Las trazas lo confirman:
la 27 agrupa por `macro_categoria`; la 35 usa `CURRENT_DATE`. Relajar D-50
a semejanza de SQL, cifras aproximadas o mismas claves con columnas extra
inflaría el saldo y no arreglaría esos KO (ni el 0,003 % de la traza 149,
que solo es tesis si un número plausible con conjunto distinto cuenta
como fallo). C-11: las cifras publicadas a 120 y a 256 no se reescriben.

**Decisión.** Se añaden **como máximo dos** ejemplos de patrón al bloque 7
del contrato, con preguntas que no están en `Preguntas.md` ni en
`evaluacion/catalogo.py` (precedente D-49: `shipped`, no `canceled`). El
primero enseña `WITH` de dos años, `GROUP BY categoria_producto`,
`es_venta_valida` y años naturales, sobre un recorte que no es la 27. El
segundo resuelve una ventana relativa contra `CAST('2018-10-17' AS DATE)`,
nunca `CURRENT_DATE`, `now()` ni `today`. No se copia el SQL de referencia
del catálogo. D-50 no se toca: execution accuracy sigue siendo igualdad
exacta de conjuntos. El contrato pasa a versión **1.2**. La pasada del
arnés es un JSON **nuevo** (`n_predict=256`, versión 1.2); no se mezcla
con `hito2_7b.json` ni con `hito2_7b_np256.json`.

**Presupuesto.** Δ estimado **~180–230 fichas** (prefijo ~3.250–3.300). El
umbral de alerta D-46 (~3.200) puede superarse; si ocurre, se recorta
prosa de notas que no viajan o una frase del bloque 4, no los ejemplos
nuevos. Si el prefijo supera ~3.400, se PARA y se pregunta: no se relaja
D-50 para «hacer sitio». Cabeza: prefijo + ~30 (pregunta) + 256 (D-61)
debe caber en 4.096. El prefill frío vuelve (D-34).

**Medición.** Prefijo 1.2 = **3.332 fichas** (7B `/tokenize`; el 1.1 sigue
en 3.067 con el mismo tokenizer). Δ real: +280 en el bloque 7, −16 en el
bloque 4 (frase de productos sin categoría). 3.332 supera ~3.200 y no
llega a ~3.400. Ocupación con pregunta y 256 de respuesta: ~3.618 de
4.096.

**Justificación.** C-08: a 7B las reglas en prosa ya pesan, pero los
patrones que el few-shot no muestra no se copian. Enseñar el grano y la
fecha de referencia es la palanca que ataca 27 y 35 sin cambiar la
métrica. Un tercer ejemplo (`unavailable` para la 44) solo entra si cabe;
el techo de dos es el que deja margen en la ventana.

**Resultado (pasada 2026-09-07, 7B, `n_predict` 256, un reintento).**
JSON nuevos: `hito2_7b_np256_d63.json` y `hito2_7b_np256_d63_n1.json`.
Contrato 1.2, 3.332 fichas. D-50 intacta.

| Bloque | 1.1 (`hito2_7b_np256` / `nivel1_7b`) | 1.2 (esta pasada) |
|---|---|---|
| Nivel 2 | 5/16 | **5/16** |
| Nivel 3 | 1/11 (solo 45) | **2/11** (35 y 45) |
| Nivel 1 | 13/18 | 12/18 |

Gana la 35: deja `CURRENT_DATE` y ancla en `CAST('2018-10-17' AS DATE)`.
La 27 pasa a `GROUP BY categoria_producto` (el grano que faltaba) pero
copia las *unidades* del few-shot en lugar de `SUM(importe_articulos)`;
D-50 sigue en KO. Mismo patrón de grano en 22 y 26, sin cruzar el
conjunto. Nivel 1: gana la 5 (`categoria_producto`); pierde la 3
(`SUM/COUNT` frente a `AVG`) y la 14 (bandera espuria). Caída de un
acierto, por debajo del umbral de revertir. No se reescriben las cifras
publicadas (C-11).

**Alternativa descartada.** Relajar D-50. Fine-tuning u otro GGUF.
`n_predict` variable por pregunta. Copiar el SQL de las preguntas 27, 35,
40, 41 o 44. Reescribir los bloques 1–6. Subir la ventana de contexto.
...
**Confirmada empíricamente el 2026-08-04** con un prefijo sintético de 2.540
fichas (prefill en frío 17.541 ms; con caché, 72–1.253 ms) y **replicada el
2026-08-08** con el prefijo real del contrato, 2.890 fichas. Ver anexo de P-07.
...
núcleos físicos. Con 4 CPU, generación ~5,5 fichas/s (2026-08-08). Con 6
CPU, ~9,6 fichas/s en el 3B de D-52. El prefill en frío de Windows nativo
(144,8 fichas/s, 2026-08-04) no es comparable al de WSL2.
...
| R-09 | El versionado público se concentra en septiembre | El calendario vive en `ROADMAP.md`; git no es el diario de diseño |
...
| P-04 | Comunicación a los tutores de las divergencias D-02, D-03, D-04 y D-11                             | **Resuelta** — enviada el 2026-07-10                   |
| P-05 | Mapeo de macro-categorías                                                                          | **Resuelta** → D-30                                    |
| P-06 | Política de retención en Delta                                                                     | **Resuelta** → D-31                                    |
| P-07 | Medición de latencia de prefill con y sin caché de prefijo, para dimensionar el contrato semántico | **Resuelta** — 2026-08-04 (sintético) y 2026-08-08 (prefijo real); ver anexo |
...
2. Construir un prefijo sintético de aproximadamente 2.000 tokens que emule el
   volumen previsto del contrato semántico. La réplica del 2026-08-08 sustituye
...
### Resultados obtenidos · 2026-08-04
...
### Resultados obtenidos · 2026-08-08 · prefijo real
...
binario ni el mismo sistema operativo que el 2026-08-04 (entonces Windows
...
2. *El tamaño extra no se lee en esta tabla.* A 145 fichas/s del 2026-08-04,
...
### Resultados obtenidos · 2026-08-12 · prefijo 1.1 (D-48 a D-51)
...
3.067 fichas (2.890 el 2026-08-08). El primer prompt, con ChatML y la
...
3. *Las magnitudes absolutas no se comparan con el 2026-08-08.* La
